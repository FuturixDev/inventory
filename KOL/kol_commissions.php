...
// 計算並更新每位 KOL 分潤
$kols = $conn->query("SELECT id, name, commission_percent, commission_start_after FROM kols")->fetch_all(MYSQLI_ASSOC);
    ...
    if ($total_qty > 0) {
        ...
        if ($check_result->num_rows > 0) {
            ...
            $insert = $conn->prepare("
                ...
                VALUES (?, ?, ?, ?, ?)
            ");
            $insert->bind_param("iidds", $kol_id, $total_qty, $total_amount, $percent, $commission_amount);
            $insert->execute();
        }
    }
}
